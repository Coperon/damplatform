// Static, dependency-free country/dial-code list for components/PhoneInput.tsx.
// No phone-number library exists in this project (grepped package.json) —
// this is plain data, not parsing/formatting logic. ISO 3166-1 alpha-2 codes;
// dial codes include the leading "+". A handful of small territories that
// share a larger neighbor's dial code (e.g. Kazakhstan/+7, Western Sahara/+212,
// the NANP Caribbean/Pacific territories under +1) are included as their own
// rows anyway, since a picker is chosen by country/flag, not by dial code.
...
const RAW_COUNTRIES: RawCountry[] = [
  ...
  { code: "VI", name: "US Virgin Islands", dial: "+1" },
...
// Regional-indicator flag emoji, derived from the ISO code rather than
// hand-typed per row (240 hand-typed flag emoji is 240 chances for a
// copy/paste mistake that's invisible in a diff) — each letter A-Z maps to
// U+1F1E6..U+1F1FF ("Regional Indicator Symbol Letter A".."Z"), and pairing
// two of them is exactly how ISO-3166 flag emoji are composed everywhere.
...
export const COUNTRIES: Country[] = RAW_COUNTRIES.map((c) => ({ ...c, flag: flagEmoji(c.code) })).sort((a, b) =>
  a.name.localeCompare(b.name),
);

// Lebanon — Coperon's primary locale, and the sensible default for a form
// most of whose invitees will be dialing/being dialed from there.
